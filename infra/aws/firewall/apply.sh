#!/usr/bin/env bash
# =============================================================================
# apply.sh — apply the biject network lockdown ON EC2 #1 (biject-enforcement).
# AWS port of infra/hetzner/firewall/apply.sh (S4-D-21).
#
# Applies, in order:
...
#
# The third layer — the security groups — is terraform's
# (infra/aws/terraform/), applied before this host even boots.
#
# Idempotent: safe to re-run; each layer replaces its own previous rules and
# never touches Docker's or the distro's rules.
#
# ACCEPTANCE: after applying, run ../firewall/verify_lockdown_aws.sh FROM THE
# AGENT HOST (EC2 #2) — the spec §5.3 probe set. The refusal must be tested
# and confirmed refused, not assumed.
# =============================================================================
set -euo pipefail
...
command -v nft >/dev/null 2>&1 || {
    ...
    echo "       (user_data/bootstrap.sh installs it; did bootstrap run?)" >&2
    exit 1
}
...
if command -v docker >/dev/null 2>&1 && command -v iptables >/dev/null 2>&1; then
    ...
    echo "         the nftables forward hook; verify from the agent host." >&2
fi
...
# ----------------------------------------------------------------------------
echo
echo "[apply] done. Now run, FROM THE AGENT HOST (EC2 #2):"
echo "        DEMO_DOMAIN=<domain> ENFORCEMENT_PRIVATE_IP=<terraform output> \\"
echo "        BIJECT_PROXY_API_KEY=<key> ./verify_lockdown_aws.sh"
